...
public class AchioteFoliagePlacer extends FoliagePlacer
{
    private final int height;

    public static final MapCodec<AchioteFoliagePlacer> CODEC = RecordCodecBuilder.mapCodec(instance ->
            foliagePlacerParts(instance)
                    .and(Codec.intRange(0,16).fieldOf("height")
                            .forGetter(fp -> fp.height))
                    .apply(instance, AchioteFoliagePlacer::new));

    public AchioteFoliagePlacer(IntProvider radius, IntProvider offset, int height)
    {
        super(radius, offset);
        this.height = height;
    }
    ...
    {
        return FoliagePlacerTypesRegister.ACHIOTE_FOLIAGE_PLACER.get();
    }
    ...
    protected void createFoliage(LevelSimulatedReader level, FoliageSetter blockSetter, RandomSource random, TreeConfiguration config, int maxFreeTreeHeight, FoliageAttachment attachment, int foliageHeight, int foliageRadius, int offset)
    {
        ...
        // range 2 => 5*5
        createLeavesLayer(level, blockSetter, random, config, attachment.pos().above(1), 1, 0,
                Arrays.asList(
                        0,1,2,
                        3,5,
                        6,7,8
                ));
        createLeavesLayer(level, blockSetter, random, config, attachment.pos().above(0), 1, 0,
                Arrays.asList(
                        0,2,
                        6,8
                ));
